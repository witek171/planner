...
-- =============================================
-- TRIGGER: SetPaidAtOnInsert
-- Opis: Ustawia kolumnę PaidAt = CreatedAt 
--       automatycznie przy INSERT, jeśli IsPaid = 1
-- Powód: uproszczenie logiki aplikacji – 
--        zapewnia spójność między IsPaid a PaidAt
-- =============================================

IF OBJECT_ID('dbo.SetPaidAtOnInsert', 'TR') IS NOT NULL
	DROP TRIGGER dbo.Reservations_SetPaidAt_Advanced;
GO

CREATE TRIGGER dbo.Reservations_SetPaidAt_Advanced
	ON Reservations
	AFTER INSERT, UPDATE
	AS
BEGIN
	...
	-- Dla INSERT: ustaw PaidAt = CreatedAt jeśli IsPaid = 1
	UPDATE r
	SET PaidAt = r.CreatedAt
	FROM Reservations r
		     INNER JOIN inserted i ON r.Id = i.Id
		     LEFT JOIN deleted d ON r.Id = d.Id
	WHERE i.IsPaid = 1
	  AND d.Id IS NULL -- To jest INSERT (nie ma odpowiadającego rekordu w deleted)
